...
if [ "$ARCH" = "arm" ] ; then
	...
	curl --output opensky-feeder.deb "https://opensky-network.org/files/firmware/opensky-feeder_latest_armhf.deb"
fi
if [ "$ARCH" = "arm64" ] ; then
    echo "Download arm64 version"
    curl --output opensky-feeder.deb "https://opensky-network.org/files/firmware/opensky-feeder_latest_arm64.deb"
fi
if [ "$ARCH" = "amd64" ] ; then
	echo "Download AMD64 version"
	curl --output opensky-feeder.deb "https://opensky-network.org/files/firmware/opensky-feeder_latest_amd64.deb"
fi
